...
function getAdminJs() {
  return "\n"
    ...
    + "\n"
    + "  // Detect a real recurring table column by how often items start at (roughly) the same x\n"
    + "  // position, rather than by gap size - gap size is unreliable when a long title can leave only\n"
    + "  // a small gap before a fixed-position location column while a short title leaves a huge one.\n"
    + "  // Among x-positions that recur often enough, the RIGHTMOST one is reliably the trailing column\n"
    + "  // (e.g. Location), since the leftmost recurring x is usually just where body text starts.\n"
    + "  var xCounts = {};\n"
    + "  items.forEach(function(it) {\n"
    + "    var rx = Math.round(it.transform[4] / 3) * 3;\n"
    + "    xCounts[rx] = (xCounts[rx] || 0) + 1;\n"
    + "  });\n"
    + "  var columnX = null;\n"
    + "  Object.keys(xCounts).forEach(function(x) {\n"
    + "    if (xCounts[x] >= 4 && Number(x) > (columnX === null ? -Infinity : columnX)) columnX = Number(x);\n"
    + "  });\n"
    + "\n"
    + "  var noisePatterns = [\n"
    ...
    + "    var cells = [], cellStartX = [];\n"
    + "    if (columnX !== null) {\n"
    + "      var before = rowItems.filter(function(it) { return it.x < columnX - 2; });\n"
    + "      var atCol = rowItems.filter(function(it) { return it.x >= columnX - 2; });\n"
    + "      if (before.length) { cells.push(before.map(function(it) { return it.str; }).join(' ').replace(/\\s+/g, ' ').trim()); cellStartX.push(before[0].x); }\n"
    + "      if (atCol.length) { cells.push(atCol.map(function(it) { return it.str; }).join(' ').replace(/\\s+/g, ' ').trim()); cellStartX.push(atCol[0].x); }\n"
    + "    } else {\n"
    + "      cells.push(rowItems.map(function(it) { return it.str; }).join(' ').replace(/\\s+/g, ' ').trim());\n"
    + "      cellStartX.push(rowItems.length ? rowItems[0].x : 0);\n"
    + "    }\n"
    + "    cells = cells.filter(Boolean);\n"
    + "    return { y: y, cells: cells, cellStartX: cellStartX, full: cells.join(' ').replace(/\\s+/g, ' ').trim(), columnX: columnX };\n"
    ...
    + "  // Date can be its own text run, OR the very start of a combined \"Date + Title\" run (common\n"
    + "  // in real files where the date was just typed at the start of the event's text box).\n"
    + "  var dateAtStartPattern = /^(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\\.?\\s+(\\d{1,2})(?:st|nd|rd|th)?(?:\\s*[-–]\\s*(\\d{1,2}))?\\s*/i;\n"
    + "  var monthHeaderPattern = /^([A-Za-z]+)\\s+EVENTS?$/i;\n"
    + "\n"
    + "  var events = [];\n"
    + "  var fallbackRows = [];\n"
    + "  var orphanLocations = [];\n"
    + "  var columnX = lines.length ? lines[0].columnX : null;\n"
    + "\n"
    + "  lines.forEach(function(line) {\n"
    + "    var cells = line.cells;\n"
    + "    if (!cells.length) return;\n"
    + "\n"
    + "    // \"AUGUST EVENTS\" / \"LOCATIONS\" section header row - not an event, skip it.\n"
    + "    if (cells[0].match(monthHeaderPattern) && cells.length <= 2) return;\n"
    + "\n"
    + "    var m = cells[0].match(dateAtStartPattern);\n"
    + "    if (m && m.index === 0) {\n"
    + "      var monthIdx = PDF_MONTH_NAMES[m[1].slice(0, 3).toLowerCase()];\n"
    + "      var day = parseInt(m[2], 10);\n"
    + "      if (monthIdx === undefined || !day) { fallbackRows.push(line); return; }\n"
    + "      var isoDate = pdfToIsoDate(currentYear, monthIdx, day);\n"
    + "      var endDate = m[3] ? pdfToIsoDate(currentYear, monthIdx, parseInt(m[3], 10)) : '';\n"
    + "      var title = cells[0].slice(m[0].length).trim() || 'Event';\n"
    + "      var location = cells.length >= 2 ? cells[1] : '';\n"
    + "      events.push({ title: title, date: isoDate, endDate: endDate, time: '', location: location, type: 'event', sourceLine: line.full, _y: line.y, _hasLoc: cells.length >= 2 });\n"
    + "      return;\n"
    + "    }\n"
    + "\n"
    + "    // A row that's ONLY location-column content (its matching date/title landed on a different\n"
    + "    // row - a real quirk seen in real files where a row's columns drift out of alignment).\n"
    + "    if (columnX !== null && cells.length === 1 && line.cellStartX[0] >= columnX - 2 && !cells[0].match(monthHeaderPattern)) {\n"
    + "      orphanLocations.push({ y: line.y, text: cells[0] });\n"
    + "      return;\n"
    + "    }\n"
    + "\n"
