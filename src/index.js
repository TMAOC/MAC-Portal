  ...
  async fetch(request, env) {
    ...
    if (path.startsWith("/api/")) {
      ...
      if (path === "/api/daily-report") {
        ...
        return jsonResponse({ date, childId, events: drData });
      }
...
function getAdminJs() {
  return "\n"
    ...
    + "    adminNewsletters = res.data.newsletters || [];\n"
    + "    adminCalendar = res.data.calendar || [];\n"
    + "    adminAdmins = res.data.admins || [];\n"
    + "    renderNewsletterAdminList();\n"
    + "    renderCalendarAdminList();\n"
    + "    renderAdminList();\n"
    + "  }).catch(function() { showNotice('Could not reach server to load admin data', 'error'); });\n"
    + "}\n"
    ...
    + "  if (!adminCalendar.length) { container.innerHTML = '<p style=\"color:var(--muted);font-size:12px;\">No events yet.</p>'; return; }\n"
    + "  var visible = calendarListExpanded ? adminCalendar : adminCalendar.slice(0, 5);\n"
    + "  var rows = visible.map(function(ev) {\n"
    + "    return '<div style=\"display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border);font-size:13px;\">'\n"
    + "      + '<span style=\"flex:1;\"><strong>' + escapeHtmlClient(ev.date) + '</strong> - ' + escapeHtmlClient(ev.title) + '</span>'\n"
    + "      + '<button style=\"background:var(--blue);padding:4px 10px;font-size:11px;\" onclick=\"openEditModal(\\'' + ev.id + '\\')\">Edit</button>'\n"
    + "      + '<button style=\"background:#D94F3D;padding:4px 10px;font-size:11px;\" onclick=\"deleteCalendarEvent(\\'' + ev.id + '\\')\">Delete</button>'\n"
    + "      + '</div>';\n"
    + "  }).join('');\n"
    + "  var toggle = '';\n"
    ...
    + "  container.innerHTML = '<p style=\"font-size:13px;color:var(--muted);\">' + adminAdmins.map(escapeHtmlClient).join(', ') + '</p>';\n"
    + "}\n"
    ...
    + "  if (!title || !date || !url) { showNotice('Please fill in title, date, and URL.', 'error'); return; }\n"
    + "  adminFetch('/api/admin/newsletters/add', { method: 'POST', body: JSON.stringify({ title: title, date: date, url: url }) }).then(function(res) {\n"
    + "    if (!res.ok || res.data.ok === false) { showNotice(res.data.error || 'Could not add newsletter', 'error'); return; }\n"
    + "    adminNewsletters = res.data.newsletters || [];\n"
    + "    renderNewsletterAdminList();\n"
    ...
    + "  adminFetch('/api/admin/newsletters/delete', { method: 'POST', body: JSON.stringify({ id: id }) }).then(function(res) {\n"
    + "    if (!res.ok || res.data.ok === false) { showNotice(res.data.error || 'Could not delete newsletter', 'error'); return; }\n"
    + "    adminNewsletters = res.data.newsletters || [];\n"
    + "    renderNewsletterAdminList();\n"
    ...
    + "  adminFetch('/api/admin/calendar/delete', { method: 'POST', body: JSON.stringify({ id: id }) }).then(function(res) {\n"
    + "    if (!res.ok || res.data.ok === false) { showNotice(res.data.error || 'Could not delete event', 'error'); return; }\n"
    + "    adminCalendar = res.data.calendar || [];\n"
    + "    renderCalendarAdminList();\n"
    ...
    + "    buckets[y].push({ x: item.transform[4], width: item.width || (item.str.length * 5), str: item.str });\n"
    + "  });\n"
    + "  var ys = Object.keys(buckets).map(Number).sort(function(a, b) { return b - a; });\n"
    + "\n"
    + "  // Figure out how big a horizontal gap has to be to count as a real column break (vs. just a\n"
    + "  // normal space between words) by looking at the typical small gap across the whole page.\n"
    + "  var allGaps = [];\n"
    + "  ys.forEach(function(y) {\n"
    + "    var rowItems = buckets[y].slice().sort(function(a, b) { return a.x - b.x; });\n"
    + "    for (var i = 0; i < rowItems.length - 1; i++) {\n"
    + "      var gap = rowItems[i + 1].x - (rowItems[i].x + rowItems[i].width);\n"
    + "      if (gap > 0 && gap < 20) allGaps.push(gap);\n"
    + "    }\n"
    + "  });\n"
    + "  allGaps.sort(function(a, b) { return a - b; });\n"
    + "  var typicalGap = allGaps.length ? allGaps[Math.floor(allGaps.length / 2)] : 3;\n"
    + "  var columnBreakThreshold = Math.max(typicalGap * 4, 10);\n"
    + "\n"
    + "  var noisePatterns = [\n"
    + "    /^\\d{1,2}\\/\\d{1,2}\\/\\d{2,4},?\\s+\\d{1,2}:\\d{2}\\s*(am|pm)/i, // browser/export print timestamp\n"
    + "    /^https?:\\/\\//i, // share link footer\n"
    + "    /^\\d+\\/\\d+$/ // bare page number like \"1/1\"\n"
    + "  ];\n"
    + "\n"
    + "  return ys.map(function(y) {\n"
    + "    var rowItems = buckets[y].slice().sort(function(a, b) { return a.x - b.x; });\n"
    + "    var cells = [], cellStartX = [], current = '';\n"
    + "    for (var i = 0; i < rowItems.length; i++) {\n"
    + "      if (i > 0) {\n"
    + "        var gap = rowItems[i].x - (rowItems[i - 1].x + rowItems[i - 1].width);\n"
    + "        if (gap > columnBreakThreshold) { cells.push(current.trim()); current = ''; }\n"
    + "        else if (current) current += ' ';\n"
    + "      }\n"
    + "      if (current === '') cellStartX.push(rowItems[i].x);\n"
    + "      current += rowItems[i].str;\n"
    + "    }\n"
    + "    if (current) cells.push(current.trim());\n"
    + "    return { y: y, cells: cells, cellStartX: cellStartX, full: cells.join(' ').replace(/\\s+/g, ' ').trim() };\n"
    + "  }).filter(function(line) {\n"
    + "    if (!line.full) return false;\n"
    + "    return !noisePatterns.some(function(p) { return p.test(line.full); });\n"
    + "  });\n"
    + "}\n"
    ...
    + "function pdfDetectYearFromLines(lines) {\n"
    + "  for (var i = 0; i < lines.length; i++) {\n"
    + "    var m = lines[i].full.match(/\\b(20\\d{2})\\b/);\n"
    + "    if (m) return parseInt(m[1], 10);\n"
    + "  }\n"
    + "  return null;\n"
    + "}\n"
    + "// Best-effort fallback for plain prose/flyer text (no real table columns in the PDF) - finds a\n"
    + "// date and optional time anywhere in the line and treats whatever's before them as the title and\n"
    + "// whatever's between/after them as the location.\n"
    + "function parseProseLine(line, currentYear) {\n"
    ...
    + "\n"
    + "  var text = line.full;\n"
    + "  var isoMatch = text.match(isoPattern);\n"
    + "  var monthMatch = text.match(monthPattern);\n"
    + "  var numMatch = text.match(numericPattern);\n"
    + "  var dateMatchObj = null, isoDate = null;\n"
    + "\n"
    + "  if (isoMatch) { isoDate = isoMatch[1] + '-' + isoMatch[2] + '-' + isoMatch[3]; dateMatchObj = isoMatch; }\n"
    + "  else if (monthMatch) {\n"
    ...
    + "    if (monthIdx !== undefined && day) { isoDate = pdfToIsoDate(year, monthIdx, day); dateMatchObj = monthMatch; }\n"
    + "  } else if (numMatch) {\n"
    ...
    + "    if (mo >= 0 && mo <= 11 && da) { isoDate = pdfToIsoDate(yr, mo, da); dateMatchObj = numMatch; }\n"
    + "  }\n"
    + "  if (!isoDate) return null;\n"
    + "\n"
    + "  var time = '', timeMatchObj = null;\n"
    + "  var rangeMatch = text.match(timeRangePattern);\n"
    + "  if (rangeMatch) {\n"
    + "    var startAmpm = rangeMatch[2] ? ' ' + rangeMatch[2].toUpperCase() : '';\n"
    + "    time = rangeMatch[1] + startAmpm + ' - ' + rangeMatch[3] + ' ' + rangeMatch[4].toUpperCase();\n"
    + "    timeMatchObj = rangeMatch;\n"
    + "  } else {\n"
    + "    var singleMatch = text.match(timeSinglePattern);\n"
    + "    if (singleMatch) { time = singleMatch[1] + ' ' + singleMatch[2].toUpperCase(); timeMatchObj = singleMatch; }\n"
    + "  }\n"
    + "\n"
    + "  var segments = [];\n"
    + "  if (dateMatchObj) segments.push({ start: dateMatchObj.index, end: dateMatchObj.index + dateMatchObj[0].length });\n"
    + "  if (timeMatchObj) segments.push({ start: timeMatchObj.index, end: timeMatchObj.index + timeMatchObj[0].length });\n"
    + "  segments.sort(function(a, b) { return a.start - b.start; });\n"
    + "\n"
    + "  var title = text.slice(0, segments[0].start);\n"
    + "  var locationParts = [];\n"
    + "  for (var k = 0; k < segments.length; k++) {\n"
    + "    var nextStart = (k + 1 < segments.length) ? segments[k + 1].start : text.length;\n"
    + "    locationParts.push(text.slice(segments[k].end, nextStart));\n"
    + "  }\n"
    + "  var location = locationParts.join(' ');\n"
    + "\n"
    + "  title = title.replace(/@/g, ' ').replace(/\\s{2,}/g, ' ').replace(/^[\\s\\-–,:;]+|[\\s\\-–,:;]+$/g, '').trim();\n"
    + "  if (!title) title = 'Event';\n"
    + "  location = location.replace(/@/g, ' ').replace(/\\s{2,}/g, ' ');\n"
    + "  location = location.replace(/^\\s*(?:location\\s*:|held at|in the|at the)\\s*/i, '');\n"
    + "  location = location.replace(/@/g, ' ').replace(/\\s{2,}/g, ' ').replace(/^[\\s\\-–,:;]+|[\\s\\-–,:;]+$/g, '').trim();\n"
    + "\n"
    + "  return { title: title, date: isoDate, endDate: '', time: time, location: location, type: 'event', sourceLine: text };\n"
    + "}\n"
    + "function parseEventsFromLines(lines) {\n"
    + "  // Many school event PDFs are really tables (Date | Event | Location columns, sometimes with a\n"
    + "  // repeating \"LOCATIONS\" header per month section). We try that structured read first, since it's\n"
    + "  // far more reliable than guessing from punctuation - and only fall back to scanning plain prose\n"
    + "  // text for PDFs that are actual flyers rather than tables.\n"
    + "  var currentYear = pdfDetectYearFromLines(lines) || new Date().getFullYear();\n"
    + "  var dayDatePattern = /^(jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t(?:ember)?)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\\.?\\s+(\\d{1,2})(?:st|nd|rd|th)?(?:\\s*[-–]\\s*(\\d{1,2}))?$/i;\n"
    + "  var monthHeaderPattern = /^([A-Za-z]+)\\s+EVENTS?$/i;\n"
    + "\n"
    + "  var events = [];\n"
    + "  var locXVotes = {};\n"
    + "  var fallbackRows = [];\n"
    ...
    + "\n"
    + "    // \"AUGUST EVENTS\" / \"LOCATIONS\" section header row - not an event, just skip it.\n"
    + "    if (cells[0].match(monthHeaderPattern) && cells.length <= 2) return;\n"
    + "\n"
    + "    var dateMatch = cells[0].match(dayDatePattern);\n"
    + "    if (dateMatch && cells.length >= 2) {\n"
    + "      var monthIdx = PDF_MONTH_NAMES[dateMatch[1].slice(0, 3).toLowerCase()];\n"
    + "      var day = parseInt(dateMatch[2], 10);\n"
    + "      if (monthIdx === undefined || !day) { fallbackRows.push(line); return; }\n"
    + "      var isoDate = pdfToIsoDate(currentYear, monthIdx, day);\n"
    + "      var endDate = dateMatch[3] ? pdfToIsoDate(currentYear, monthIdx, parseInt(dateMatch[3], 10)) : '';\n"
    + "      var title = cells[1] || 'Event';\n"
    + "      var location = cells[2] || '';\n"
    + "      if (cells.length >= 3) {\n"
    + "        var rx = Math.round(line.cellStartX[2] / 5) * 5;\n"
    + "        locXVotes[rx] = (locXVotes[rx] || 0) + 1;\n"
    + "      }\n"
    + "      events.push({ title: title, date: isoDate, endDate: endDate, time: '', location: location, type: 'event', sourceLine: line.full, _y: line.y, _hasLoc: cells.length >= 3 });\n"
    + "      return;\n"
    + "    }\n"
    + "\n"
    ...
    + "  });\n"
    + "\n"
    + "  // Some rows' Location text drifts a few points out of alignment with its matching Date/Event\n"
    + "  // row in the source PDF (a real quirk seen in real files), landing on its own single-cell row.\n"
    + "  // Re-attach those to the nearest event in the same location column instead of losing them.\n"
    + "  var bestX = null, bestCount = 0;\n"
    + "  Object.keys(locXVotes).forEach(function(x) { if (locXVotes[x] > bestCount) { bestCount = locXVotes[x]; bestX = Number(x); } });\n"
    + "  if (bestX !== null) {\n"
    + "    var orphanLocations = [];\n"
    + "    fallbackRows = fallbackRows.filter(function(line) {\n"
    + "      if (line.cells.length === 1 && !line.cells[0].match(dayDatePattern) && !line.cells[0].match(monthHeaderPattern)) {\n"
    + "        var rx = Math.round(line.cellStartX[0] / 5) * 5;\n"
    + "        if (Math.abs(rx - bestX) <= 5) { orphanLocations.push({ y: line.y, text: line.cells[0] }); return false; }\n"
    + "      }\n"
    + "      return true;\n"
    + "    });\n"
    + "    events.filter(function(e) { return !e._hasLoc; }).forEach(function(ev) {\n"
    + "      var best = null, bestDist = Infinity;\n"
    + "      orphanLocations.forEach(function(loc) {\n"
    + "        if (loc.used) return;\n"
    + "        var dist = Math.abs(loc.y - ev._y);\n"
    + "        if (dist < bestDist) { bestDist = dist; best = loc; }\n"
    + "      });\n"
    + "      if (best && bestDist <= 10) { ev.location = best.text; best.used = true; }\n"
    + "    });\n"
    + "  }\n"
    + "  events.forEach(function(e) { delete e._y; delete e._hasLoc; });\n"
    + "\n"
    + "  fallbackRows.forEach(function(line) {\n"
    + "    var ev = parseProseLine(line, currentYear);\n"
    ...
    + "  var ev = adminCalendar.filter(function(e) { return e.id === id; })[0];\n"
    + "  if (!ev) return;\n"
    ...
    + "    renderCalendarAdminList();\n"
    + "    closeEditModal();\n"
    + "    showNotice('Event updated.', 'success');\n"
    + "  }).catch(function() { showNotice('Could not reach server.', 'error'); });\n"
    + "}\n"
    + "function bulkImportParents() {\n"
    + "  var raw = document.getElementById('bulk-csv').value;\n"
    + "  var merge = document.getElementById('bulk-merge').checked;\n"
    + "  var lines = raw.split('\\n').map(function(l) { return l.trim(); }).filter(Boolean);\n"
    + "  var rows = lines.map(function(line) {\n"
    + "    var parts = line.split(',').map(function(p) { return p.trim(); });\n"
    + "    return { email: parts[0] || '', childIds: parts.slice(1).filter(Boolean) };\n"
    + "  });\n"
    + "  if (!rows.length) { showNotice('Paste at least one row of CSV data.', 'error'); return; }\n"
    + "  adminFetch('/api/admin/parents/import', { method: 'POST', body: JSON.stringify({ rows: rows, merge: merge }) }).then(function(res) {\n"
    + "    if (!res.ok || res.data.ok === false) { showNotice(res.data.error || 'Import failed', 'error'); return; }\n"
    + "    var r = res.data.results;\n"
    + "    var html = '<p><strong>' + r.imported + '</strong> imported, <strong>' + r.skipped + '</strong> skipped.</p>';\n"
    + "    if (r.errors && r.errors.length) html += '<p style=\"color:var(--red);\">' + r.errors.map(escapeHtmlClient).join('<br>') + '</p>';\n"
    + "    document.getElementById('bulk-import-results').innerHTML = html;\n"
    + "    showNotice('Bulk import complete.', 'success');\n"
    + "  }).catch(function() { showNotice('Could not reach server.', 'error'); });\n"
    + "}\n"
    + "function addNewParent() {\n"
    + "  var emails = [document.getElementById('new-parent-email1').value, document.getElementById('new-parent-email2').value, document.getElementById('new-parent-email3').value].map(function(e) { return e.trim(); }).filter(Boolean);\n"
    + "  var childIds = [document.getElementById('new-child-id1').value, document.getElementById('new-child-id2').value, document.getElementById('new-child-id3').value, document.getElementById('new-child-id4').value].map(function(c) { return c.trim(); }).filter(Boolean);\n"
    + "  var limited = document.getElementById('new-parent-limited').checked;\n"
    ...
    + "  if (!email || !childId) { showNotice('Please fill in parent email and child ID.', 'error'); return; }\n"
    + "  adminFetch('/api/admin/parents/add-child', { method: 'POST', body: JSON.stringify({ email: email, childId: childId }) }).then(function(res) {\n"
    + "    if (!res.ok || res.data.ok === false) { showNotice(res.data.error || 'Could not add child', 'error'); return; }\n"
    + "    document.getElementById('add-child-results').innerHTML = '<p style=\"color:var(--green);\">Child added to ' + escapeHtmlClient(email) + '.</p>';\n"
    + "    document.getElementById('add-child-email').value = '';\n"
    + "    document.getElementById('add-child-id').value = '';\n"
    + "    showNotice('Child added.', 'success');\n"
    + "  }).catch(function() { showNotice('Could not reach server.', 'error'); });\n"
    + "}\n"
    + "function deleteParent() {\n"
    + "  var email = document.getElementById('delete-parent-email').value.trim();\n"
    + "  if (!email) { showNotice('Please enter a parent email.', 'error'); return; }\n"
    + "  if (!window.confirm('Delete all access for ' + email + '? This cannot be undone.')) return;\n"
    + "  adminFetch('/api/admin/parents/delete', { method: 'POST', body: JSON.stringify({ email: email }) }).then(function(res) {\n"
    + "    if (!res.ok || res.data.ok === false) { showNotice(res.data.error || 'Could not delete parent', 'error'); return; }\n"
    + "    document.getElementById('delete-parent-results').innerHTML = '<p style=\"color:var(--green);\">Deleted ' + escapeHtmlClient(email) + '.</p>';\n"
    ...
    + "  parentListExpanded = !parentListExpanded;\n"
    + "  renderParentListResults();\n"
    + "}\n"
    + "function loadParentList() {\n"
    + "  adminFetch('/api/admin/parents/list').then(function(res) {\n"
    + "    if (!res.ok || res.data.ok === false) { showNotice(res.data.error || 'Could not load parents', 'error'); return; }\n"
    + "    adminParents = res.data.parents || [];\n"
    ...
    + "  if (!email) { showNotice('Please enter an email.', 'error'); return; }\n"
    + "  adminFetch('/api/admin/admins/add', { method: 'POST', body: JSON.stringify({ email: email }) }).then(function(res) {\n"
    + "    if (!res.ok || res.data.ok === false) { showNotice(res.data.error || 'Could not add admin', 'error'); return; }\n"
    + "    adminAdmins = res.data.admins || [];\n"
    + "    renderAdminList();\n"
...
.main { padding:12px 16px; padding-bottom:100px; max-width:700px; margin:0 auto; }
...
.action-card { background:none; border:none; border-radius:0; padding:0; margin-bottom:8px; text-align:center; }
.action-card h3 { font-family:Nunito,sans-serif; color:var(--muted); font-size:11px; font-weight:700; text-transform:uppercase; letter-spacing:1px; }
.action-card p { color:var(--muted); font-size:11px; margin-top:2px; opacity:.8; }
