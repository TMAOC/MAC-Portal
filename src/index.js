...
function getAdminJs() {
  return "\n"
    ...
    + "    }\n"
    + "\n"
    + "    // Split the line by position rather than just deleting matched substrings - flyers read\n"
    + "    // Title ... Date ... Time ... Location, so anything before the first date/time match is the\n"
    + "    // title, and anything between/after the date and time matches is the location. This is what\n"
    + "    // fixes location text (e.g. \"Front Lawn\", \"MAC Gym\") ending up stuck inside the title instead\n"
    + "    // of its own field, even when the PDF has no explicit \"Location:\" label at all.\n"
    + "    var segments = [];\n"
    ...
    + "\n"
    + "    var title = line.slice(0, segments[0].start);\n"
    + "    var locationParts = [];\n"
    + "    for (var k = 0; k < segments.length; k++) {\n"
    + "      var nextStart = (k + 1 < segments.length) ? segments[k + 1].start : line.length;\n"
    + "      locationParts.push(line.slice(segments[k].end, nextStart));\n"
    + "    }\n"
